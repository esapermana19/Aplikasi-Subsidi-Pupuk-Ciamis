...
<body class="bg-white text-gray-800 p-5 w-full max-w-[380px] mx-auto text-[10px]">
    ...
    <div class="flex flex-col gap-3 mb-4">
        <div class="flex justify-between">
            <div class="w-1/2 pr-2">
                ...
                <h3 class="text-[11px] font-bold text-gray-900 leading-tight">{{ $transaksi->petani->nama_petani ?? '-' }}</h3>
                <p class="text-[9px] text-gray-500 mt-0.5 break-all">NIK: {{ $transaksi->petani->nik ?? '-' }}</p>
            </div>
        ...
        <div class="flex justify-between mt-1">
            <div class="w-1/2 pr-2">
                ...
                <h3 class="text-[11px] font-bold text-gray-900 leading-tight">{{ $transaksi->mitra->nama_mitra ?? '-' }}</h3>
                <p class="text-[9px] text-gray-500 mt-0.5">No: {{ $transaksi->mitra->nomor_mitra ?? '-' }}</p>
            </div>
            <div class="w-1/2 pl-2 text-right">
        ...
        <div class="border border-gray-100 rounded-lg overflow-hidden shadow-sm">
            <table class="w-full text-left text-[9px]">
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach($transaksi->rincian as $detail)
                        <tr>
                            <td class="px-2 py-2 text-gray-900 font-bold">{{ $detail->pupuk->nama_pupuk }}</td>
                            <td class="px-2 py-2 text-gray-500">{{ number_format($detail->harga_satuan, 0, ',', '.') }}</td>
                            <td class="px-2 py-2 text-center text-gray-900 font-bold">{{ $detail->jumlah }} Kg</td>
            ...
            <div class="bg-gray-50/50 px-2 py-2 flex justify-between items-center border-t border-gray-100">
                <span class="font-black text-gray-900 uppercase text-[10px] tracking-wide">Total</span>
    ...
    {{-- Status Badge Bottom --}}
    <div class="mt-4 bg-gray-50 rounded-lg p-3 flex flex-col items-center justify-center border border-gray-100 text-center gap-1.5">
        <div class="text-[8px] font-bold text-gray-400 uppercase tracking-widest">Status Pengambilan</div>
        @if($transaksi->status_pengambilan == 'sudah')
            <div class="flex items-center gap-1.5 text-emerald-600 font-bold bg-emerald-50 px-3 py-1.5 rounded-md text-[10px]">
        ...
        @else
            <div class="flex items-center gap-1.5 text-amber-600 font-bold bg-amber-50 px-3 py-1.5 rounded-md text-[10px]">
