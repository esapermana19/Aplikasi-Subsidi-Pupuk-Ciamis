...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('mitra.pupuk_tersedia') }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded font-semibold text-sm">
            + Permintaan Baru
        </a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <table class="min-w-full leading-normal">
            <thead>
                <tr>
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tanggal Permintaan</th>
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Catatan</th>
                ...
                <tr>
                    ...
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        <span class="px-3 py-1 font-semibold rounded-full text-xs
                            {{ $req->status_permintaan == 'pending' ? 'bg-yellow-100 text-yellow-800' :
                              ($req->status_permintaan == 'disetujui' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800') }}">
                            {{ ucfirst($req->status_permintaan) }}
                        </span>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">
                        <button onclick="lihatDetail({{ $req->id_permintaan }}, '{{ $req->status_permintaan }}')"
...
<div id="modalDetail" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-1/2 p-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <div class="flex justify-end mt-6 border-t pt-4">
            <button onclick="closeModal()" class="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 font-medium">Tutup</button>
        </div>
    </div>
</div>
...
<script>
    function lihatDetail(id_permintaan, status) {
        ...
        if (status === 'pending') {
            badge.className = 'mb-4 inline-block px-3 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800';
        } else if (status === 'disetujui') {
            badge.className = 'mb-4 inline-block px-3 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800';
        } else {
            badge.className = 'mb-4 inline-block px-3 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800';
        }
